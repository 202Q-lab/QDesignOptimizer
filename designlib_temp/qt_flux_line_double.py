...
from src.utils.utils import get_middle_point

class QTFluxLineDouble(QComponent,QTComponent):
        default_options = Dict(connector_cpw_ext = '25um',
                           flux_cpw_ext = '25um',
                           flux_cpw_width = '12um',
                           flux_cpw_gap = '10um',
                           flux_gap = '3um',
        ...
        def make(self):
            ...
            def get_single_flux_line(mirror: bool):
                ...
                before_bend_left = (p.squid_pos_x - 0.5 * p.flux_width, flux_y_bottom + p.squid_side * (p.flux_line_length + p.flux_width))
                before_bend_right = (p.squid_pos_x + 0.5 * p.flux_width, flux_y_bottom + p.squid_side * (p.flux_line_length + p.flux_width))
                ...
                angles = np.linspace(0, arc_angle_rad, num=5, endpoint=True)  # Increase num for a smoother arc
                angles_left = np.flip(angles[1:])
                angles_right = angles[1:]
                arc_points_left = [(arc_center[0] + arc_radius_left * np.cos(angle), arc_center[1] - arc_radius_left * np.sin(angle)) for angle in angles_left]
                arc_points_right = [(arc_center[0] + arc_radius_right * np.cos(angle), arc_center[1] - arc_radius_right * np.sin(angle)) for angle in angles_right]
                width_diff = (p.flux_cpw_width - p.flux_width) / 2 # half at each side

                wide_left = (arc_points_left[0][0] - np.sin(arc_angle_rad) * p.flux_taper_length - np.cos(arc_angle_rad) * width_diff, 
                                arc_points_left[0][1] - np.cos(arc_angle_rad) * p.flux_taper_length + np.sin(arc_angle_rad) * width_diff)
                wide_right = (arc_points_right[-1][0] - np.sin(arc_angle_rad) * p.flux_taper_length + np.cos(arc_angle_rad) * width_diff,
                                arc_points_right[-1][1] - np.cos(arc_angle_rad) * p.flux_taper_length - np.sin(arc_angle_rad) * width_diff)
                end_left = (wide_left[0] - np.sin(arc_angle_rad) * p.flux_cpw_ext,
                ...
                flux_cpw_extension_top_half = draw.Polygon([
                    (np.mean([wide_left[0], end_left[0]]), np.mean([wide_left[1], end_left[1]])),
                    end_left,
                    end_right,
                    (np.mean([wide_right[0], end_right[0]]), np.mean([wide_right[1], end_right[1]])),
                ...
                flux_tip = draw.Polygon([
                    (p.squid_pos_x - 0.5 * p.flux_width - p.flux_gap - p.flux_rounding,
                        flux_y_bottom),
                    (p.squid_pos_x,
                        flux_y_bottom),
                    (p.squid_pos_x,
                ...
                arc_points_left_pocket = [(arc_center[0] + arc_radius_left_pocket * np.cos(angle), arc_center[1] - arc_radius_left_pocket * np.sin(angle)) for angle in angles_left]
                arc_points_right_pocket = [(arc_center[0] + arc_radius_right_pocket * np.cos(angle), arc_center[1] - arc_radius_right_pocket * np.sin(angle)) for angle in angles_right]
                sin_shift_pocket = np.sin(arc_angle_rad) * p.flux_cpw_gap
                cos_shift_pocket = np.cos(arc_angle_rad) * p.flux_cpw_gap
                wide_left_pocket = (wide_left[0] - cos_shift_pocket, wide_left[1] + sin_shift_pocket)
                wide_right_pocket = (wide_right[0] + cos_shift_pocket, wide_right[1] - sin_shift_pocket)
                ...
                flux_line_pocket = draw.Polygon([
                    ...
                    (p.squid_pos_x + 0.5 * p.flux_width + p.flux_gap,
                        flux_y_bottom + p.squid_side * (
                            p.flux_line_length + p.flux_width)),
                ...
                flux_line_end_hard_corner = draw.Polygon([
                    ...
                    (np.mean([wide_left[0], end_left[0]]) - cos_shift, np.mean([wide_left[1], end_left[1]]) + sin_shift),
                    end_left_pocket,
                    end_right_pocket,
                    (np.mean([wide_right[0], end_right[0]]) + cos_shift, np.mean([wide_right[1], end_right[1]]) - sin_shift),
                ...
                def mirror_in_x_axis(shapes: list):
                    return [draw.shapely.ops.transform(lambda x, y: (-x, y), shape) for shape in shapes]
                if mirror:
            ...
            flux_line_left, flux_line_pin_left, flux_line_pocket_left = get_single_flux_line(mirror=False)
            flux_line_right, flux_line_pin_right, flux_line_pocket_right = get_single_flux_line(mirror=True)
            ...
            self.add_qgeometry('poly', {'flux_line_pocket_left': flux_line_pocket_left}, subtract = True)
            self.add_qgeometry('poly', {'flux_line_pocket_right': flux_line_pocket_right}, subtract = True)
            # Add all pins to qgeometry
            self.add_pin('flux_left', flux_line_pin_left.coords, p.flux_cpw_width, input_as_norm=True)
            self.add_pin('flux_right', flux_line_pin_right.coords, p.flux_cpw_width, input_as_norm=True)
