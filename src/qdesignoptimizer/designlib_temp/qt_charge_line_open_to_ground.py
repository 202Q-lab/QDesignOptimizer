...
class QTChargeLineOpenToGround(QComponent, QTComponent):
    r"""Tapered CPW segment ending in an open to ground.
    ...
    def make(self):
        ...
        y_top = p.end_line_length + p.cpw_end_width + p.taper_length + p.cpw_ext
        # Draw the taper and tip
        tapered_line = draw.Polygon(
            [
                (0.5 * p.cpw_end_width, 0),
                (0.5 * p.cpw_end_width, (p.end_line_length + p.cpw_end_width)),
                (0.5 * p.cpw_width, y_top - 1 * p.cpw_ext),
                (0.5 * p.cpw_width, y_top),
                (-0.5 * p.cpw_width, y_top),
                (-0.5 * p.cpw_width, y_top - 1 * p.cpw_ext),
                (-0.5 * p.cpw_end_width, (p.end_line_length + p.cpw_end_width)),
                (-0.5 * p.cpw_end_width, p.cpw_end_width),
                (-0.5 * p.cpw_end_width, p.cpw_end_width),
                (-0.5 * p.cpw_end_width, 0),
            ]
        )
        ...
        cpw_extension_top_half = draw.Polygon(
            [
                (
                    +0.5 * p.cpw_width,
                    (p.end_line_length + p.cpw_end_width + p.taper_length + p.cpw_ext),
                ),
                (
                    +0.5 * p.cpw_width,
                    (
        ...
        tapered_line = draw.union(tapered_line, cpw_extension_top_half)
        # Generate the line pin to connect the CPW to
        tapered_line_pin = draw.LineString(
            [(-1 * 0.5 * p.cpw_width, y_top), (+1 * 0.5 * p.cpw_width, y_top)]
        )

        # Draw the line pocket
        tapered_line_pocket = draw.Polygon(
            [
                (0.5 * p.cpw_end_width + p.taper_gap, -p.taper_gap),
                (
                    0.5 * p.cpw_end_width + p.taper_gap,
                    (p.end_line_length + p.cpw_end_width),
                ),
                (0.5 * p.cpw_width + p.cpw_gap, y_top - 1 * p.cpw_ext),
                (0.5 * p.cpw_width + p.cpw_gap, y_top),
                (-0.5 * p.cpw_width - p.cpw_gap, y_top),
                (-0.5 * p.cpw_width - p.cpw_gap, y_top - 1 * p.cpw_ext),
                (
                    -0.5 * p.cpw_end_width - p.taper_gap,
        ...
        tapered_line_pocket_cpw_top = draw.Polygon(
            [
                (0.5 * p.cpw_width + p.cpw_gap, y_top),
                (0.5 * p.cpw_width + p.cpw_gap, y_top - 0.5 * 1 * p.cpw_ext),
                (-0.5 * p.cpw_width - p.cpw_gap, y_top - 0.5 * 1 * p.cpw_ext),
                (-0.5 * p.cpw_width - p.cpw_gap, y_top),
            ]
        )
        ...
        self.add_qgeometry("poly", {"tapered_line": tapered_line})
        self.add_qgeometry(
            "poly", {"tapered_line_pocket": tapered_line_pocket}, subtract=True
        )
