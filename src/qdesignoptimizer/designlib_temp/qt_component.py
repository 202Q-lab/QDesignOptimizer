...
class QTComponent:
    ...
    def central_coordinates(self):
        ...
        start_pin_points = self._get_connected_pin(
            self.options.pin_inputs.start_pin
        ).points[-2::]
        ...
        end_pin_points = self._get_connected_pin(
            self.options.pin_inputs.end_pin
        ).points[-2::]
        ...
        p.orientation = np.rad2deg(
            np.arctan((end_pin_y - start_pin_y) / (end_pin_x - start_pin_x))
        )
    ...
    def log_route(self):
        ...
        ptsfilename = (
            f"{self.design.gds_path}{self.design.chip_name}_" f"{self.p.route_type}.txt"
        )
    ...
    def round_object(self, input_polygon, rounding):
        ...
        width.
        """
        return input_polygon.buffer(rounding).buffer(-2 * rounding).buffer(rounding)
    ...
    def add_air_bridge_coordinates(self, airbridge_coordinates: List[list]):
        ...
        assert (
            len(airbridge_coordinates) == 2
        ), "The airbridge coordinates should have 1 start and 1 end point."
        assert (
            len(airbridge_coordinates[0]) == 3
        ), "The start point should have x,z,y coordinates."
        assert (
            len(airbridge_coordinates[1]) == 3
        ), "The end point should have x,z,y coordinates."
