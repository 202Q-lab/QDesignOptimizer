...
class QTCoupledLineTee(QComponent, QTComponent):
    """The QTCoupledLineTee class extends CoupledLineTee with QTComponent and optimization methods
    ...
    Default Options:
        ...
        * prime_length_1: '200um' -- Prime length to e.g. put 50 Ohm termination away from the coupling
        * prime_length_2: '200um' -- Prime length to e.g. put 50 Ohm termination away from the coupling
    ...
    default_options = Dict(
        prime_width="10um",
        prime_gap="6um",
        prime_length_1="300um",
        prime_length_2="300um",
        second_width="10um",
        second_gap="6um",
        coupling_space="3um",
    ...
    def make(self):
        ...
        prime_cpw_length = p.coupling_length
        second_flip = 1
        if p.mirror:
            second_flip = -1

        # Primary CPW
        prime_cpw = draw.LineString(
            [
                [-prime_cpw_length / 2 - p.prime_length_1, 0],
                [prime_cpw_length / 2 + p.prime_length_2, 0],
            ]
        )
        ...
        self.add_qgeometry(
            "path",
            {"prime_cpw_sub": prime_cpw},
            width=p.prime_width + 2 * p.prime_gap,
            subtract=True,
        )
        self.add_qgeometry(
            "path", {"second_cpw": second_cpw}, width=p.second_width, fillet=p.fillet
        )
        self.add_qgeometry(
            "path",
            {"second_cpw_sub": second_cpw_etch},
