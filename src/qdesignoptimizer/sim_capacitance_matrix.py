...
class CapacitanceMatrixStudy():
    ...
    def __init__(
        ...
        self.capacitance_matrix_fF:pd.DataFrame = None  # (fF) gets populated by simulate_capacitance_matrix
        self.mode_capacitances_matrix_fF:dict = None    # (fF) gets populated by simulate_capacitance_matrix
    ...
    def simulate_capacitance_matrix(
        ...
        lom_analysis.sim.run(components=self.component_names, open_terminations=self.open_pins)
...
class ModeDecayIntoChargeLineStudy(CapacitanceMatrixStudy):
    ...
    def get_t1_limit_due_to_decay_into_charge_line(self):
        ...
        """
        assert self.capacitance_matrix_fF is not None, "capacitance_matrix_fF is not set, you need to run .simulate_capacitance_matrix()."
        ...
        if isinstance(self.mode_capacitance_name, str): # single island, grounded design
            name_island = self.mode_capacitance_name

            Csum = np.abs(self.capacitance_matrix_fF.loc[name_island, name_island])
            Ccoupling = np.abs(self.capacitance_matrix_fF.loc[name_island, charge_line_name])

            self.t1_limit_due_to_decay_into_charge_line = calculate_t1_limit_grounded_lumped_mode_decay_into_chargeline(
        ...
        elif isinstance(self.mode_capacitance_name, list) and len(self.mode_capacitance_name)==2 and all(isinstance(item, str) for item in self.mode_capacitance_name): # split transmon, floating
            ...
            Ca1 = np.abs(self.capacitance_matrix_fF.loc[name_island_A, charge_line_name])
            Cb1 = np.abs(self.capacitance_matrix_fF.loc[name_island_B, charge_line_name])
            CJ = np.abs(self.capacitance_matrix_fF.loc[name_island_A, name_island_B])

            self.t1_limit_due_to_decay_into_charge_line = calculate_t1_limit_floating_lumped_mode_decay_into_chargeline(
            ...
            raise NotImplementedError("The mode capacitance name must be a string or a list of string matching the name of the island(s).")
...
def sim_capacitance_matrix(
    ...
    lom_analysis.sim.run(components=component_names, open_terminations= open_terminations)
