...
def add_design_variables_to_design(design: DesignPlanar, design_variables: dict[str:str]):
...
def name_cav_to_coupler(branch_number: int, coupler_number: Union[str, int, None]=None):
    return f'NAME_CAV{branch_number}_TO_COUPLER{coupler_number}'

def name_lp_to_tee(lp_branch_number: Union[str, int], tee_branch_number: Union[str, int]):
    return f'NAME_LP{lp_branch_number}_TO_TEE{tee_branch_number}'

def name_tee_to_tee(tee_branch_number1: Union[str, int], tee_branch_number2: Union[str, int]):
