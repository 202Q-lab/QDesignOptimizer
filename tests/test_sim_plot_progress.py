...
class TestOptPltSet:
    ...
    def test_init_with_invalid_scale(self):
        """Test initialization with invalid scale values."""
        # Should accept any string value for scales, matplotlib will validate later
        plt_set = OptPltSet("x_param", "y_param", x_scale="invalid", y_scale="also_invalid")
        
        assert plt_set.x_scale == "invalid"
        assert plt_set.y_scale == "also_invalid"
    
    def test_init_with_complex_variables(self):
        """Test initialization with more complex variable names."""
        complex_x = "very_long_variable_name_with_underscores"
        complex_y = ["nested.variable.name", "another[0].indexed.variable", "third-variable-with-dashes"]
        
        plt_set = OptPltSet(complex_x, complex_y)
        
        assert plt_set.x == complex_x
        assert plt_set.y == complex_y
        assert plt_set.x_label == complex_x
        assert plt_set.y_label == complex_y
...
def mock_plot_settings():
    ...
    return {
        "Frequencies": [
            OptPltSet(ITERATION, [qubit_freq, resonator_freq], "Iteration", "Frequency (Hz)"),
        ],
        "Cross-Kerr vs LJ": [
...
class TestPlotProgress:
    ...
    def test_plot_progress(self, mock_show, mock_subplots, mock_figure, 
        ...
        assert mock_subplots.call_count == len(mock_plot_settings), "Wrong number of subplots created"
        assert mock_fig.suptitle.call_count == len(mock_plot_settings), "Wrong number of titles set"
        assert mock_show.call_count == 1, "plt.show() should be called once"

    @patch('matplotlib.pyplot.close')
    @patch('matplotlib.pyplot.subplots')
    @patch('matplotlib.pyplot.show')
    def test_plot_progress_with_iteration(self, mock_show, mock_subplots, mock_close,
                                        mock_optimization_results, mock_system_target_params):
    ...
    @patch('matplotlib.pyplot.subplots')
    @patch('matplotlib.pyplot.show')
    def test_plot_progress_with_design_variable(self, mock_show, mock_subplots,
                                             mock_optimization_results, mock_system_target_params):
    ...
    @patch('matplotlib.pyplot.subplots')
    @patch('matplotlib.pyplot.show')
    def test_plot_progress_multiple_panels(self, mock_show, mock_subplots,
                                        mock_optimization_results, mock_system_target_params):
        """Test plotting with multiple panels in a single figure."""
        # Mock the subplot and axes objects
        mock_fig = MagicMock()
        mock_axes = MagicMock()
        mock_subplots.return_value = (mock_fig, [mock_axes, mock_axes])  # Return list of axes
        ...
        plot_settings = {
            "Frequency Progress": [
                ...
                OptPltSet(ITERATION, resonator_freq, "Iteration", "Resonator Frequency (Hz)"),
            ]
        }
    ...
    @patch('matplotlib.pyplot.subplots')
    @patch('matplotlib.pyplot.show')
    def test_plot_progress_with_missing_data(self, mock_show, mock_subplots,
                                          mock_optimization_results, mock_system_target_params):
    ...
    @patch('matplotlib.pyplot.subplots')
    @patch('matplotlib.pyplot.show')
    def test_plot_progress_log_scale(self, mock_show, mock_subplots,
                                  mock_optimization_results, mock_system_target_params):
...
def realistic_optimization_data():
    ...
    plot_settings = {
        ...
        "Linewidths": [
            OptPltSet(ITERATION, [qubit_kappa, resonator_kappa], "Iteration", "Linewidth (Hz)", y_scale="log"),
        ],
        "Nonlinearities": [
            OptPltSet(ITERATION, [anharmonicity, cross_kerr], "Iteration", "Nonlinearity (Hz)"),
        ],
        "Design Variables": [
            OptPltSet(ITERATION, "design_var_lj_qubit_1", "Iteration", "Inductance (nH)"),
            OptPltSet(ITERATION, "design_var_width_qubit_1", "Iteration", "Width (um)"),
        ],
        "Performance vs Design": [
            OptPltSet("design_var_lj_qubit_1", qubit_freq, "Inductance (nH)", "Frequency (Hz)"),
            OptPltSet("design_var_coupl_length_resonator_1_qubit_1", cross_kerr, "Coupling Length (um)", "Cross-Kerr (Hz)"),
        ]
    }
...
class TestGetDataFromParameter:
    """Tests for the get_data_from_parameter function used inside plot_progress."""
    
    @patch('qdesignoptimizer.sim_plot_progress.get_value_and_unit')
    def test_get_data_design_variable(self, mock_get_value_and_unit, mock_optimization_results):
        """Test extracting design variable values."""
        from qdesignoptimizer.sim_plot_progress import plot_progress
        
        # Setup mock return value for get_value_and_unit
        mock_get_value_and_unit.return_value = (10.0, "nH")
        
        # Create a test subfunction that mimics the internal get_data_from_parameter
        def get_data_wrapper(param_name, result):
            # This is simplified from the actual implementation in plot_progress
            if param_name == ITERATION:
                return 1  # Just return a fixed value for testing
                ...
                val, _ = get_value_and_unit(result["design_variables"][param_name])
                return val
            else:
                return None
        
        # Test with a design variable
        design_var = "design_var_lj_qubit_1"
        result = mock_optimization_results[0]
        
        data = get_data_wrapper(design_var, result)
        
        # Verify get_value_and_unit was called correctly
        mock_get_value_and_unit.assert_called_once_with(result["design_variables"][design_var])
        assert data == 10.0
    
    def test_get_data_missing_parameter(self, mock_optimization_results):
        ...
        nonexistent_param = "does_not_exist"
        result = mock_optimization_results[0]
        
        data = get_data_wrapper(nonexistent_param, result)
...
class TestPlotProgressIntegration:
    ...
    @patch('matplotlib.pyplot.subplots')
    @patch('matplotlib.pyplot.show')
    def test_realistic_data(self, mock_show, mock_subplots, mock_figure, realistic_optimization_data):
    ...
    @patch('matplotlib.pyplot.subplots')
    @patch('matplotlib.pyplot.show')
    def test_empty_results(self, mock_show, mock_subplots, mock_figure, realistic_optimization_data):
        ...
        assert mock_subplots.call_count == 5, "Should create figures even with empty results"
        assert mock_show.call_count == 1, "plt.show() should be called once"

    @patch('matplotlib.pyplot.figure') 
    @patch('matplotlib.pyplot.subplots')
    @patch('matplotlib.pyplot.show')
    def test_inconsistent_keys(self, mock_show, mock_subplots, mock_figure, realistic_optimization_data):
        ...
        inconsistent_results = realistic_optimization_data["optimization_results"].copy()
